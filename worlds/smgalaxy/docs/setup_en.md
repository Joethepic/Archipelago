...
2. Acquire the most recent APWorld from the [Releases page](https://github.com/Joethepic/Archipelago/releases/latest) and place the smgalaxy.apworld in the custom_worlds folder of your Archipelago install
...
2. Open the Archipelago Launcher and click "Generate". This will create a zip file in Archipelago/output
   - You will need to open this .zip to get your .apsmg patch file if you are not using the Archipelago website
...
6. Run the ArchipelagoLauncher.exe and click `Open Patch`. Select your `.apsmg` patch file.
You will be prompted to locate your Super Mario Galaxy ISO the first time you do this.
   - This action will automatically run the Super Mario Galaxy Client (and connect to the webhost if the patch was downloaded from there).
   - The patch will be placed in the same folder as your patch file by default.
   - You will ***not*** need to patch the game every time, and can simply run the `SMGClient` from the list on the right of the Archipelago Launcher
to continue later.
7. Open Dolphin and from Dolphin, open your newly patched Super Mario Galaxy ISO. Load all the way into a brand new save file, and pause.
...
Then you can click connect on the SMG Client so long as the port matches what is shown on the webpage.
...
the host will provide you with either a link to download your patch file, or with an APSMG patch file that they have extracted from the zip. 
...
to locate your Super Mario Galaxy ISO

### Connect to the client

When the ISO patched, the Super Mario Galaxy client (SMGClient) should have also automatically launched in
...
1. Open Dolphin and from Dolphin, open your newly patched Super Mario Galaxy ISO
2. In the server page, there will be a port number. Copy this port number into the top of your SMGClient. 
   - The field should read `archipelago.gg:<port number>`
3. Once you have loaded into the game, the client should log that Dolphin has been connected. Click the `Connect` button
at the top of the SMGClient. If the port number is correct, you are now connected and ready to play!
